...
const Groups = () => {
  ...
  const validateIcon = (file: File): Promise<string | null> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        if (!ALLOWED_ICON_SIZES.includes(img.width)) return resolve('Icon must be exactly 256×256 or 512×512');
        resolve(null);
      };
      img.onerror = () => {
        URL.revokeObjectURL(url);
  ...
  const onIconChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) {
      setIconError('Use PNG, JPEG, or WebP');
      return;
    }
  ...
  return (
    <div className="max-w-[940px] mx-auto">
      <div className="flex items-center justify-between gap-3 flex-wrap mb-5">
        <h1 className="text-3xl font-bold flex items-center gap-3" style={{ fontFamily: 'Orbitron, sans-serif', color: 'hsl(180 100% 95%)', textShadow: '0 0 16px hsl(180 100% 50% / 0.5)' }}>
      ...
      {showCreate && (
        <div className="rounded-xl border border-primary/30 p-5 mb-6" style={{ background: 'linear-gradient(135deg, hsl(260 40% 10%) 0%, hsl(260 35% 6%) 100%)', boxShadow: '0 0 30px hsl(180 100% 50% / 0.1)' }}>
          <h2 className="font-bold text-lg mb-4">New Group</h2>
          <div className="grid grid-cols-1 md:grid-cols-[160px_1fr] gap-5">
            <div className="text-center">
              <div className="rounded-md border-2 border-dashed border-primary/30 mx-auto flex items-center justify-center overflow-hidden" style={{ width: 140, height: 140, background: 'hsl(260 40% 14%)' }}>
                {iconPreview ? <img src={iconPreview} alt="Icon preview" className="w-full h-full object-cover" /> : <ImageIcon className="w-8 h-8 text-primary/50" />}
              </div>
              <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={onIconChange} className="hidden" />
              <Button variant="outline" onClick={() => fileRef.current?.click()} className="mt-2 w-full text-xs">Upload Icon</Button>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea id="g-desc" value={description} onChange={(e) => setDescription(e.target.value)} maxLength={500} className="w-full min-h-[100px] px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="What is your group about?" />
              </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-muted-foreground"><Users className="w-12 h-12 mx-auto opacity-30 mb-3" /><p>No groups yet — be the first to create one!</p></div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {groups.map((g) => (
            <Link key={g.id} to={`/groups/${g.id}`} className="rounded-xl border border-primary/20 p-4 hover:border-primary/50 transition-colors" style={{ background: 'hsl(260 40% 10%)' }}>
              <div className="flex gap-3">
                <div className="rounded-md overflow-hidden flex-shrink-0" style={{ width: 64, height: 64, background: 'hsl(260 40% 14%)' }}>
                  {g.icon_url ? <img src={g.icon_url} alt={g.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-primary/40"><Crown className="w-6 h-6" /></div>}
                </div>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1"><Users className="w-3 h-3" />{g.member_count} member{g.member_count !== 1 ? 's' : ''}</div>
                </div>
              </div>
            </Link>
          ))}
